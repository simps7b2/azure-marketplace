...
:github: https://github.com/simps7b2/azure-marketplace
:current: 8.8
:version: 8.8.1
...
verification. For this reason, `xpack.monitoring.elasticsearch.ssl.verification_mode` is set to `none`.
...
deploys a free perpetual basic license, providing access to the {subscriptions}[basic features]. 
These versions use a distribution with Elastic Stack features included in the installation, 
and the license applied determines which features are activated.
+
The default is `Yes`.
